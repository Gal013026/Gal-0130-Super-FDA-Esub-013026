...
# ============================================================
# 8) Default TW datasets + default guidance (original + 6 mock sets)
# ============================================================
...
DEFAULT_GUIDANCE_ORIGINAL = """# 預審/形式審查指引（Mock）—第二等級輸入一般醫材：一次性使用無菌注射器（含針）
...
# ---- 6 mock datasets + guidance (from prior results) ----
# (A) Dataset: Suture (import, Class II)
CASES_SUTURE = [
    {
...
GUIDE_SUTURE = """# 預審/形式審查指引（Mock）—第二等級輸入一般醫材：外科縫合針線組
## 0. 審查目的
...
# (B) Dataset: HbA1c IVD change (domestic, Class III)
CASES_HBA1C_CHANGE = [
    {
...
GUIDE_HBA1C_CHANGE = """# 預審/形式審查指引（Mock）—第三等級國產 IVD：HbA1c 定量檢測（HPLC 法）變更案

## 0. 審查目的
確認變更案申請之變更範圍描述、受影響文件清單、性能可比性證據與文件版本控管是否齊備。

---

## 1. 必要文件清單（預期應附）
1. 變更案申請書（含原許可證字號/案號、變更內容摘要）
2. QMS/QSD 或 ISO 13485 證明（在效期內；範圍涵蓋 IVD 設計/製造）
3. 變更評估報告（原因、內容、影響分析、風險評估更新）
4. 受影響文件版本清單（IFU/Label/技術檔案章節等）
5. 標示/IFU 差異對照表（新舊版本逐條比對）
6. 性能可比性與驗證摘要（依變更性質決定）
7. 穩定性資料摘要（配方/包裝/儲存條件影響）
8. 風險管理文件摘要（ISO 14971；變更後更新）
9. 上市後監督與抱怨處理資料摘要（如相關）

---

## 2. 申請書關鍵欄位檢核
- 原許可證字號/前案號是否可追溯
- 變更類型與範圍是否明確（配方/供應商/標示/包裝等）
- 變更影響判定是否合理且有風險更新結論
...
# (C) Dataset: Blood pressure monitor (domestic, Class II)
CASES_BP = [
    {
        ...
        "preclinical_replace": "",
        "clinical_just": "不適用",
        "clinical_info": "成熟技術裝置；以性能驗證、電氣安全/EMC 與風險管理支持，無臨床試驗需求。"
    },
    {
        ...
        "preclinical_replace": "",
        "clinical_just": "不適用",
        "clinical_info": "成熟血壓量測技術；新增無線功能以資安與通訊驗證支持，不需臨床試驗。"
    }
]

GUIDE_BP = """# 預審/形式審查指引（Mock）—第二等級國產一般醫材：上臂式電子血壓計（含藍牙型）

## 0. 審查目的
確認申請書基本欄位、型號與規格範圍、必備附件、標示與技術摘要一致性，以及（若含無線/軟體）相關文件是否齊備。

---

## 1. 必要文件清單（預期應附）
1. 申請書（分類分級、製造廠資訊）
2. QMS/QSD 或 ISO 13485（在效期內；範圍涵蓋設計/製造）
3. 中文標籤/說明書/外盒標示擬稿（含型號、序號/批號、注意事項、保養、警語）
4. 技術檔案摘要（原理、量測範圍/精度、壓脈帶尺寸、使用環境條件）
5. 性能驗證摘要（精度、重複性、漂移/校正策略）
6. 電氣安全與 EMC 測試摘要（依適用標準）
7.（若具藍牙/無線）無線通訊與共存評估摘要（如適用）
8.（若具 App/軟體）軟體版本資訊與版本控管摘要
9.（若具連線功能）資安/隱私風險管理摘要
10. 風險管理摘要（ISO 14971）
...
- 缺電氣安全/EMC 測試摘要
- 型號/規格範圍描述不清（壓脈帶尺寸、供電、無線差異）
- 藍牙型缺資安/隱私風險管理摘要
"""

# (D) Dataset: Peripheral IV catheter (import, Class II)
CASES_IV_CATHETER = [
    {
        ...
        "preclinical_replace": "",
        "clinical_just": "不適用",
        "clinical_info": "成熟技術導管；以臨床前性能/滅菌/風險管理支持，無臨床試驗需求。"
    },
    {
...
GUIDE_IV_CATHETER = """# 預審/形式審查指引（Mock）—第二等級輸入一般醫材：周邊靜脈導管（含安全型）
...
1. 申請書
2. 醫療器材商許可執照
3. 原廠授權登記書（涵蓋全部規格與包裝型式）
4. 出產國製售證明（CFS；可辨識產品與效期）
5. ISO 13485（範圍涵蓋設計/製造）
6. 中文標籤/說明書/外盒標示擬稿（無菌、一次性、EO、效期、批號）
7. 技術檔案摘要（材料、規格表、製程）
8. EO 滅菌確效摘要（ISO 11135；SAL、放行方式）
9. 生物相容性摘要（ISO 10993）
10. 性能測試摘要（流量/漏液/抗折；安全型需可靠度）
...
- 授權書未涵蓋安全型或部分規格
- CFS 過期或缺產品可辨識資訊
- EO 滅菌確效摘要未提 SAL/放行方式
- 安全機構缺可靠度/啟動成功率摘要
"""

# (E) Dataset: Respiratory multiplex RT-PCR IVD (import, Class III)
CASES_RESP_VIRUS = [
    {
        ...
        "manu_note": "原廠負責設計、製造與最終放行；臺灣醫療器材商負責進口與上市後監督。",
        "auth_applicable": "適用",
        "auth_desc": "附原廠授權登記書及中譯本；授權範圍涵蓋本套組與配套耗材（如適用）。",
        "cfs_applicable": "適用",
        "cfs_desc": "提供出產國製售證明或同等效力文件影本，含簽發機關、效期與產品可辨識資訊。",
        "qms_applicable": "適用",
        "qms_desc": "ISO 13485:2016 證書有效期至 2027-11；範圍涵蓋 IVD 試劑設計與製造。",
        "similar_info": "提供與既有多重 RT-PCR 檢測之比較：標的基因區域、檢測通道配置、內控策略、相容儀器差異。",
        "labeling_info": "IFU/標籤擬稿含：用途、樣本型別、操作步驟、限制、結果判讀、品質控制、儲存條件、效期、批號。",
        "tech_file_info": "提供方法學摘要、性能規格（LoD、特異性）、相容儀器清單、干擾/交叉反應評估摘要與版本控管摘要。",
        "preclinical_info": "分析性能摘要：LoD、精密度/再現性、交叉反應/干擾、內控有效性、陰/陽性控制策略摘要。",
        "preclinical_replace": "",
        "clinical_just": "適用",
        "clinical_info": "提供臨床性能摘要：樣本、研究設計、敏感度/特異度或一致性等（摘要）。"
    },
    {
        ...
        "manu_country": "EU (Member State)",
        "manu_addr": "BioPark 8, 69120 Heidelberg, Germany",
        "manu_note": "作為配套萃取試劑；與檢測試劑共用品質系統與版本控管。",
        "auth_applicable": "適用",
        "auth_desc": "授權文件包含本萃取試劑與配套平台適用性說明；附中譯本。",
        "cfs_applicable": "適用",
        "cfs_desc": "同系列產品之製售證明文件影本，含效期與簽發機關。",
        "qms_applicable": "適用",
        "qms_desc": "ISO 13485:2016 證書有效期至 2027-11。",
        "similar_info": "提供與同類核酸萃取試劑比較：平台相容性、萃取效率、抑制物去除策略。",
        "labeling_info": "IFU/標籤擬稿：樣本型別、步驟、儲存條件、效期、批號、注意事項。",
        "tech_file_info": "提供平台相容性摘要、抑制物評估摘要與版本控管摘要。",
        "preclinical_info": "性能摘要：萃取效率、抑制物影響、批間差與穩定性摘要。",
        "preclinical_replace": "",
        "clinical_just": "不適用",
...
GUIDE_RESP_VIRUS = """# 預審/形式審查指引（Mock）—第三等級輸入 IVD：多重呼吸道病毒 RT-PCR（含配套萃取）
...
## 1. 必要文件清單（預期應附）
1. 申請書（第三等級 IVD；用途/樣本型別/相容儀器）
2. 醫療器材商許可執照（一致性）
3. 原廠授權登記書（涵蓋檢測與配套萃取/耗材）
4. CFS 或同等效力文件（產品可辨識、效期、簽發機關）
5. ISO 13485（涵蓋 IVD 設計/製造）
6. IFU/標籤擬稿（用途、判讀、限制、QC、儲存、效期、批號）
7. 技術檔案摘要（方法學、內控策略、相容儀器/平台）
8. 分析性能摘要（LoD、精密度、交叉反應、干擾、污染控制）
9. 臨床性能摘要（研究設計、樣本數、敏感度/特異度或一致性）
10. 風險管理摘要（ISO 14971）
11. 文件版本控管清單（版本號/日期/修訂摘要）

---

## 4. 常見缺失
- 缺臨床性能摘要或不可追溯研究設計/樣本數/比較方法
- 相容儀器/平台界定不清
...
# (F) Dataset: Electrosurgical pencil + electrode (domestic, Class II)
CASES_ELECTROSURG = [
    {
        ...
        "tech_file_info": "提供端子相容性規格、材料清單、結構圖、製程概述、委外滅菌受控摘要、包裝完整性摘要。",
        "preclinical_info": "測試摘要：絕緣耐壓相關設計驗證、熱效應/功能驗證、端子連接可靠度、包裝完整性、滅菌確效與生物相容性摘要。",
        "preclinical_replace": "",
        "clinical_just": "不適用",
        "clinical_info": "成熟電外科附件；以性能/安全設計驗證、滅菌與風險管理支持，無臨床試驗需求。"
    },
    {
        ...
        "manu_country": "TAIWAN， ROC",
        "manu_addr": "高雄市前鎮區新生路66號5樓",
        "manu_note": "電極加工與檢驗由同廠執行；EO 滅菌委外受控；與電燒筆共用版本控管。",
        "auth_applicable": "不適用",
        "auth_desc": "",
        "cfs_applicable": "不適用",
        "cfs_desc": "",
        ...
        "labeling_info": "標示擬稿含：相容型號、一次性使用警語、EO、效期、批號與注意事項。",
        "tech_file_info": "提供尺寸規格與公差摘要、材料/表面處理摘要、裝配相容性摘要、包裝完整性與滅菌確效摘要。",
        "preclinical_info": "測試摘要：尺寸/表面品質、裝配牢固度、導電性/熱效應設計驗證（依用途）、生物相容性與滅菌/包裝摘要。",
        "preclinical_replace": "",
        "clinical_just": "不適用",
...
GUIDE_ELECTROSURG = """# 預審/形式審查指引（Mock）—第二等級國產一般醫材：一次性電燒筆與配套電極（單極）

## 0. 審查目的
確認電外科附件之申請書欄位、相容性界定（主機/端子/型號）、滅菌與包裝、標示警語、以及基本性能/安全驗證摘要是否齊備。

---

## 1. 必要文件清單（預期應附）
1. 申請書（分類分級、用途、製造廠資訊）
2. ISO 13485（在效期內；範圍涵蓋設計/製造）
3. 中文標籤/說明書/外盒標示擬稿（一次性警語、相容主機注意事項、EO、效期、批號）
4. 技術檔案摘要（材料、端子規格、刀頭/電極型式）
5. 相容性界定摘要（相容主機或端子規格、使用模式限制）
6. EO 滅菌確效摘要（ISO 11135；SAL/放行方式/委外受控）
7. 包裝完整性摘要
8. 生物相容性摘要（ISO 10993，接觸材料）
9. 性能與安全設計驗證摘要（連接可靠度、絕緣/耐壓、功能驗證）
10. 風險管理摘要（ISO 14971；熱傷害、錯配等）
...
DEFAULT_TW_CASESETS: Dict[str, Dict[str, Any]] = {
    "tw_default_syringe_single": {"title": "Default Case (Syringe, single)", "cases": [DEFAULT_CASE_ORIGINAL]},
    "tw_mock_suture_import": {"title": "Mock Dataset: Surgical Suture (Import, Class II)", "cases": CASES_SUTURE},
    "tw_mock_hba1c_change_domestic": {"title": "Mock Dataset: HbA1c IVD Change (Domestic, Class III)", "cases": CASES_HBA1C_CHANGE},
    "tw_mock_bp_domestic": {"title": "Mock Dataset: Blood Pressure Monitor (Domestic, Class II)", "cases": CASES_BP},
    "tw_mock_iv_catheter_import": {"title": "Mock Dataset: Peripheral IV Catheter (Import, Class II)", "cases": CASES_IV_CATHETER},
    "tw_mock_resp_virus_ivd_import": {"title": "Mock Dataset: Respiratory Virus RT-PCR IVD (Import, Class III)", "cases": CASES_RESP_VIRUS},
    "tw_mock_electrosurg_domestic": {"title": "Mock Dataset: Electrosurgical Pencil & Tips (Domestic, Class II)", "cases": CASES_ELECTROSURG},
}

DEFAULT_TW_GUIDANCES: Dict[str, Dict[str, str]] = {
    "tw_guidance_syringe_original": {"title": "Guidance: Syringe (Import, Class II)", "md": DEFAULT_GUIDANCE_ORIGINAL},
    "tw_guidance_suture": {"title": "Guidance: Suture (Import, Class II)", "md": GUIDE_SUTURE},
    "tw_guidance_hba1c_change": {"title": "Guidance: HbA1c Change (Domestic, Class III)", "md": GUIDE_HBA1C_CHANGE},
    "tw_guidance_bp": {"title": "Guidance: BP Monitor (Domestic, Class II)", "md": GUIDE_BP},
    "tw_guidance_iv_catheter": {"title": "Guidance: Peripheral IV Catheter (Import, Class II)", "md": GUIDE_IV_CATHETER},
    "tw_guidance_resp_virus": {"title": "Guidance: Respiratory Virus RT-PCR (Import, Class III)", "md": GUIDE_RESP_VIRUS},
    "tw_guidance_electrosurg": {"title": "Guidance: Electrosurgical Accessories (Domestic, Class II)", "md": GUIDE_ELECTROSURG},
}


# ============================================================
# 9) Guidance struct conversion + merge/diff utilities
# ============================================================
...
# ============================================================
# 10) TW dataset standardization (rule-mapping prioritized, skip failed rows)
# ============================================================
...
# ============================================================
# 11) Agents YAML loading (+ standardization as in original)
# ============================================================
...
# ============================================================
# 16) TW Premarket helpers: session <-> dict
# ============================================================
...
# ============================================================
# 18) TW Field Mapping Dictionary Editor (upload/download/edit)
# ============================================================
...
# ============================================================
# 19) TW Premarket Tab (updated with dataset + guidance manager)
# ============================================================
...
# ============================================================
# 20) 510(k) Intelligence tab (kept)
# ============================================================
def render_510k_tab():
...
# ============================================================
# 22) 510(k) Review Pipeline tab (updated with default checklist+guidance management)
# ============================================================

DEFAULT_510K_CHECKLIST_SETS: Dict[str, Dict[str, Any]] = {
...
DEFAULT_510K_GUIDANCES: Dict[str, Dict[str, str]] = {
    "k510_guidance_review_memo": {
        "title": "Guidance: Internal 510(k) Review Memo (Mock)",
        "md": """# 510(k) Review Pipeline Guidance (Mock) — Internal Use
...
# ============================================================
# 23) Note Keeper (kept; trimmed for brevity of this updated file)
# ============================================================
